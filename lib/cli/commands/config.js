...
const { DIFFICULTY_STEPS, getDifficultyLabel } = require(path.join(__dirname, '../../difficulty'));
...
function show(options) {
  ...
  if (!isSetMode) {
    ...
    info('  Difficulty:     ' + getDifficultyLabel(multiplier));
...
function set(key, value) {
  ...
  if (key === 'environment') {
    ...
    if (isNaN(num) || !DIFFICULTY_STEPS.includes(num)) {
      error('Invalid multiplier: ' + value + '. Valid values: ' + DIFFICULTY_STEPS.join(', '));
      process.exitCode = 1;
      return;
    }

    if (!config.difficulty) config.difficulty = {};
    config.difficulty.multiplier = num;
    saveConfig(configPath, config);
    success('Difficulty set to: ' + getDifficultyLabel(num));
    process.exitCode = 0;

  } else if (key === 'sensitivity') {
    const valid = ['strict', 'normal', 'relaxed'];
    if (!valid.includes(value)) {
      error('Invalid sensitivity: ' + value + '. Valid values: ' + valid.join(', '));
      process.exitCode = 1;
      return;
    }

    if (!config.detection) config.detection = {};
    config.detection.sensitivity = value;
    saveConfig(configPath, config);
    success('Detection sensitivity set to: ' + value);
    process.exitCode = 0;

  } else {
    error('Unknown config key: ' + key + '. Settable keys: environment, multiplier, sensitivity');
    process.exitCode = 1;
  }
}
...
function get(key) {
  ...
  if (key === 'environment') {
    ...
    info(String(config.difficulty?.multiplier ?? 1.0));
    process.exitCode = 0;
  } else if (key === 'sensitivity') {
    info(config.detection?.sensitivity ?? 'normal');
    process.exitCode = 0;
  } else {
    error('Unknown config key: ' + key + '. Gettable keys: environment, multiplier, sensitivity');
    process.exitCode = 1;
  }
}
