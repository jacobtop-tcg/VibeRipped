...
function loadPool() {
  ...
  try {
    ...
    if (e.code === 'ENOENT') {
      error('No pool found. Run setup first: viberipped config --kettlebell');
    } else {
 ...
 * @param {string} repsStr - Reps value as string (optional for batch mode)
...
function add(name, repsStr, options = {}) {
  // Detect batch mode (comma-separated format)
  if (name.includes(',')) {
    return addBatch(name, options);
  }

  // Single-add mode requires reps parameter
  if (repsStr === undefined) {
    error('Missing reps parameter', 'viberipped pool add "Exercise name" <reps>');
    process.exit(1);
  }
  ...
  if (exerciseType !== 'reps' && exerciseType !== 'timed') {
    error('Invalid type (must be "reps" or "timed")', 'viberipped pool add "Exercise name" <reps> --type <reps|timed>');
    process.exit(1);
  }
  ...
  if (options.duration !== undefined) {
    ...
    if (isNaN(parsed) || parsed < 1 || parsed > 999) {
      error('Invalid duration (must be integer 1-999)', 'viberipped pool add "Exercise name" <reps> --duration <seconds>');
      process.exit(1);
    }
  ...
  if (options.environments !== undefined) {
    ...
    if (options.environments === '' || options.environments.trim().length === 0) {
      error('Invalid environments (provide comma-separated list)', 'viberipped pool add "Exercise" 15 --environments "home,office"');
      process.exit(1);
    }
...
/**
 * Batch add command - adds multiple exercises from comma-separated input.
 *
 * @param {string} batchInput - Comma-separated string: "Name1 reps1, Name2 reps2, ..."
 * @param {Object} [options={}] - Options (unused in batch mode - defaults applied)
 */
function addBatch(batchInput, options = {}) {
  ...
  if (items.length === 0) {
    error('Invalid batch format (no exercises found)', 'viberipped pool add "Name1 reps1, Name2 reps2, ..."');
    process.exit(1);
  }

  // Parse each item into {name, reps}
  const parsed = [];
  for (const item of items) {
    const parts = item.split(/\s+/);

    // Need at least 2 parts (name + reps)
    if (parts.length < 2) {
      error(`Invalid format for "${item}" (missing reps)`, 'viberipped pool add "Name1 reps1, Name2 reps2, ..."');
      process.exit(1);
    }

    // Last part is reps, everything else is name
    const repsStr = parts[parts.length - 1];
    const nameStr = parts.slice(0, -1).join(' ');

    // Validate name
    const validatedName = validateExerciseName(nameStr);
    if (validatedName === null) {
      error(`Invalid exercise name "${nameStr}" (must be 1-50 characters)`, 'viberipped pool add "Name1 reps1, Name2 reps2, ..."');
      process.exit(1);
    }
    ...
    const validatedReps = validateReps(repsStr);
    if (validatedReps === null) {
      error(`Invalid reps "${repsStr}" for "${validatedName}" (must be integer 1-999)`, 'viberipped pool add "Name1 reps1, Name2 reps2, ..."');
      process.exit(1);
    }
  ...
  for (const item of parsed) {
    ...
    if (batchNames.has(nameLower)) {
      error(`Duplicate exercise in batch: "${item.name}"`);
      process.exit(1);
    }
  ...
  for (const item of parsed) {
    ...
    const duplicate = pool.find(ex => ex.name.toLowerCase() === nameLower);
    if (duplicate) {
      error(`Exercise "${item.name}" already exists in pool`);
      process.exit(1);
    }
  }

  // All validation passed - add all exercises
  for (const item of parsed) {
    pool.push({
      name: item.name,
      reps: item.reps,
...
function remove(name) {
  ...
  if (!name || typeof name !== 'string' || name.trim().length === 0) {
    error('Exercise name required', 'viberipped pool remove "Exercise name"');
    process.exit(1);
  }
...
/**
 * Manage command - interactive checklist to toggle exercises on/off.
 */
async function manage() {
  ...
  // Build checkbox choices with type-aware display labels
  const choices = pool.map((exercise, index) => {
    ...
    const label = `${display} [${equipment}] (${envs})`;
  ...
  info('Navigate with arrow keys, space to toggle, enter to save, escape to cancel.\n');

  // Show CheckboxPrompt
  let selected;
  try {
    const prompt = new CheckboxPrompt('Toggle exercises (Space to toggle, Enter to confirm):', choices);
    selected = await prompt.prompt();
  } catch (e) {
    // Handle escape (rejection)
    console.log('\nPool management cancelled.');
    process.exitCode = 0;
    return;
  }

  // Filter pool to selected indices
  ...
  success(`Pool updated: ${newPool.length} of ${pool.length} exercises kept`);
  info(`Removed ${removed} exercise(s) from rotation.`);
  info('Rotation index reset to beginning.');
