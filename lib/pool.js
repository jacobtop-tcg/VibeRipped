...
/**
 * Assembles exercise pool based on available equipment and environment.
 *
 * Filtering logic:
 * - Always include bodyweight exercises (equipment = [])
 * - Include equipment exercise if ALL required equipment is available (AND logic)
 * - Filter by environment: include if exercise has "anywhere" or matches active environment
 * - If environment filter empties result, fall back to equipment-only pool with warning
 ...
 * @param {string} [environment='anywhere'] - Active environment filter (office, home, coworking, anywhere)
...
function assemblePool(equipmentConfig, environment = 'anywhere') {
  ...
  // Fail-safe: if environment filter empties result, fall back to equipment-filtered pool
  if (envFiltered.length === 0) {
    console.error(`Environment filter '${environment}' produced empty pool, falling back to equipment-only pool`);
    return equipmentFiltered;
  }

  // Additional fail-safe: should never happen since bodyweight exercises always included
  if (envFiltered.length === 0) {
