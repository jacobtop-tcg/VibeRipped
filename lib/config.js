...
function validateConfig(config) {
  ...
  // Validate detection field if present (v1.1 schema extension)
  if (config.detection !== undefined) {
    if (typeof config.detection !== 'object' || config.detection === null) {
      return false;
    }
    // sensitivity must be string if present
    if (config.detection.sensitivity !== undefined &&
        typeof config.detection.sensitivity !== 'string') {
      return false;
    }
    // durationThreshold must be number or null if present
    if (config.detection.durationThreshold !== undefined &&
        config.detection.durationThreshold !== null &&
        typeof config.detection.durationThreshold !== 'number') {
      return false;
    }
    // fallbackOnError must be boolean if present
    if (config.detection.fallbackOnError !== undefined &&
        typeof config.detection.fallbackOnError !== 'boolean') {
      return false;
    }
  }
...
function loadConfig(configPath) {
  try {
    ...
    const normalized = {
      ...
      detection: {
        sensitivity: config.detection?.sensitivity ?? "normal",
        durationThreshold: config.detection?.durationThreshold ?? null,
        fallbackOnError: config.detection?.fallbackOnError ?? true
      },
