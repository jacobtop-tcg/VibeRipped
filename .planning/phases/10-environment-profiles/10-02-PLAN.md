...
Replace the flat `config` command with a Commander command group supporting three modes:

1. `vibripped config` (no args) -- show current config (existing behavior)
2. `vibripped config set <key> <value>` -- set a config value
3. `vibripped config get <key>` -- get a config value

Implementation: Change the config command from a flat `.command('config')` to a command group:
```javascript
...
IMPORTANT: The existing `vibripped config --kettlebell` equipment flag behavior MUST continue working. The `.action()` on the base config command handles both show mode (no flags) and equipment set mode (with flags). Check if any equipment flags are set to determine mode, exactly as the current configHandler does.

**lib/cli/commands/config.js changes:**

Refactor from single default export to named exports: `show`, `set`, `get`.

- `show(options)`: Current configHandler logic -- check for equipment flags, if none show config, if flags present update equipment. Add environment display to show mode: `info('  Environment:  ' + config.environment);`
- `set(key, value)`: Handle `key === 'environment'` -- validate value is non-empty string, load config, update environment field, save config via saveConfig(). Print success: `success('Environment set to: ' + value)`. For unknown keys, print error: `error('Unknown config key: ' + key + '. Settable keys: environment')`.
- `get(key)`: Handle `key === 'environment'` -- load config, print `info(config.environment)`. For unknown keys, print error.

Module export changes:
```javascript
module.exports = { show, set, get };
// NOT: module.exports = configHandler;
```
