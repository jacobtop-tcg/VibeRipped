...
CURRENT CODE in engine.js (lines 82-84, config-driven mode block):
```javascript
...
CHANGE TO (two distinct steps -- extract environment FIRST, then pass to assemblePool):
```javascript
const config = loadConfig(configPath);
const environment = config.environment || 'anywhere';
const assembledPool = assemblePool(config, environment);
const assembledHash = computePoolHash(assembledPool);
```

Step-by-step:
1. Line 82 stays the same: `const config = loadConfig(configPath);`
2. ADD new line after it: `const environment = config.environment || 'anywhere';`
3. Change line 83 from `assemblePool(config)` to `assemblePool(config, environment)`
4. Line 84 stays the same: `const assembledHash = computePoolHash(assembledPool);`

The `config.environment` field comes from configuration.json via loadConfig() in lib/config.js. loadConfig already normalizes it: `environment: config.environment || "anywhere"` (see lib/config.js line 151). The `|| 'anywhere'` fallback in engine.js is a defense-in-depth for legacy configs that predate the environment field.

That's it for the code change. The existing bounds check (`if (state.currentIndex >= actualPool.length)`) already handles index reset. The existing pool hash change detection handles pool size changes.
