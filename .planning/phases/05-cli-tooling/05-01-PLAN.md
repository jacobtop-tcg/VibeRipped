...
must_haves:
  truths:
    ...
    - "User can run vibripped config --kettlebell and pool.json is generated immediately"
  ...
  artifacts:
    ...
    - path: "lib/cli/commands/config.js"
      provides: "Config command: set/show equipment flags and generate pool"
      min_lines: 40
  ...
  key_links:
    ...
    - from: "lib/cli/commands/config.js"
      to: "lib/pool.js"
      via: "assemblePool + computePoolHash for immediate pool generation after config save"
...
<task type="auto">
  <name>Task 2: Config command implementation with pool generation</name>
  <files>lib/cli/commands/config.js</files>
  <action>
Create lib/cli/commands/config.js that wraps existing lib/config.js and lib/pool.js:
...
3. **Set mode** (at least one flag specified):
   ...
   - **Generate pool immediately after saving config** (satisfies SC-1: "declare equipment AND generate pool"):
     a. Import `assemblePool` and `computePoolHash` from `lib/pool.js` via `require(path.join(__dirname, '../../pool'))`.
     b. Import `getStateDir` from `lib/state.js` via `require(path.join(__dirname, '../../state'))`.
     c. Call `assemblePool(newConfig)` to get the filtered exercise array based on equipment flags.
     d. Compute `configPoolHash` via `computePoolHash(assembledPool)`.
     e. Compute `poolHash` via `computePoolHash(assembledPool)` (same value since pool is freshly assembled).
     f. Write pool.json to `path.join(getStateDir(), 'pool.json')` using write-file-atomic with mode 0o600 and 2-space JSON indent. Create directory with `fs.mkdirSync(stateDir, { recursive: true, mode: 0o700 })` first.
     g. Load existing state.json if it exists (try/catch for ENOENT). If it exists, update `state.configPoolHash = configPoolHash`, `state.poolHash = poolHash`, and `state.currentIndex = 0` (reset rotation since pool changed). Write updated state.json via write-file-atomic.
     h. If state.json does not exist, that is fine -- it will initialize on next trigger() call with the correct pool.
   - Print confirmation: "Configuration updated" with equipment summary.
   - Print "Pool generated with {N} exercises." showing assembledPool.length.
   - Exit with code 0.

4. Error handling: If saveConfig fails silently (it logs to stderr), the CLI should still exit 0 since saveConfig doesn't throw. If validateConfig returns false, print error and exit 1. If pool.json write fails, log warning but do not exit 1 (pool will regenerate on next trigger).
...
NOTE: Commander maps --pull-up-bar to options.pullUpBar automatically (camelCase conversion). Verify this maps correctly to config.equipment.pullUpBar.
  ...
  <verify>
    Run `node bin/vibripped.js config` to show defaults (all disabled). Run `node bin/vibripped.js config --kettlebell --dumbbells` and verify it reports updated AND prints pool generation message. Verify pool.json now exists in the state directory (check `ls ~/.config/viberipped/pool.json` or equivalent XDG path) and contains kettlebell + dumbbell + bodyweight exercises. Run `node bin/vibripped.js config` again and verify kettlebell/dumbbells show enabled. Run `node bin/vibripped.js config --no-kettlebell` and verify kettlebell reverts to disabled AND pool.json is regenerated (should no longer contain kettlebell exercises).
  </verify>
  <done>Config command shows current equipment when called with no flags. Sets equipment when called with flags, supports --no-{flag} negation, persists to configuration.json. Generates pool.json immediately after any equipment change, with correct exercise filtering based on equipment flags. State rotation index resets when config changes.</done>
</task>
...
2. `node bin/vibripped.js config --kettlebell` updates configuration.json, generates pool.json with kettlebell + bodyweight exercises, and reports success
3. `node bin/vibripped.js config` shows current equipment state
4. `node bin/vibripped.js config --no-kettlebell` disables equipment and regenerates pool.json without kettlebell exercises
5. pool.json exists and contains valid exercise array after running config with any equipment flag
6. `node bin/vibripped.js test` shows exercise without changing state.json
7. `node bin/vibripped.js test && node bin/vibripped.js test` shows same exercise both times
8. `node --test` (existing tests) still pass -- no regressions from engine.js change
9. `node bin/vibripped.js` with no args shows help
</verification>
...
- Config command generates pool.json immediately when equipment flags are set (SC-1: declare equipment AND generate pool in one command)
