...
<task type="auto">
  <name>Task 1: Wire detection config into statusline.js</name>
...
3. Construct detection state path from config dir:
   ...
   ```
   **Verification:** `getConfigPath()` returns `~/.config/viberipped/config.json` (from lib/config.js). `path.dirname()` of that is `~/.config/viberipped/`. Therefore `detectionStatePath` resolves to `~/.config/viberipped/detection-state.json`. Confirm this by checking that `getConfigPath()` returns a path ending in `/config.json` within the viberipped config directory. If the config path structure ever changes, this will track it automatically since it derives from the same `getConfigPath()` base.
...
5. Keep the rest of statusline.js unchanged (latency extraction, engine trigger, formatting)
  ...
  <verify>
    `node -e "const {getConfigPath} = require('./lib/config'); const path = require('path'); const p = path.join(path.dirname(getConfigPath()), 'detection-state.json'); console.log(p); process.assert(p.endsWith('.config/viberipped/detection-state.json'))"` confirms correct path resolution.
    `node --test test/statusline.test.js` passes existing tests (no regressions from wiring change).
  </verify>
  <done>
    statusline.js loads config via loadConfig(getConfigPath()), constructs detectionStatePath as ~/.config/viberipped/detection-state.json, and passes both to isProcessing.
  </done>
</task>

<task type="auto">
  <name>Task 2: Update integration tests for delta-based detection</name>
...
7. Update each integration test's input JSON to include `session_id` and `cost` fields for realism, except the fallback test which deliberately omits `cost`.
  ...
  <done>
    Integration tests validate delta detection end-to-end, fallback path tested, session reset tested. All existing tests still pass.
  </done>
</task>

<task type="checkpoint:human-verify" gate="blocking">
  <name>Task 3: Live testing of detection accuracy</name>
  <files>statusline.js</files>
  <action>
