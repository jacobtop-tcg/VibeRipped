...
Implement a `getThreshold(config)` helper function inside `detection.js` that maps sensitivity strings to millisecond values:

```javascript
function getThreshold(config) {
  const SENSITIVITY_MAP = { strict: 50, normal: 100, relaxed: 500 };
...
5. Call `getThreshold(options.config)` to resolve threshold (strict=50ms, normal=100ms, relaxed=500ms, or custom durationThreshold override)
